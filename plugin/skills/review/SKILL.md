...
```bash
node "${GROK_PLUGIN_ROOT}/scripts/grok-companion.mjs" review --target '<target from $ARGUMENTS>' [other non-task flags from $ARGUMENTS, each substituted value single-quoted] --task-file - <<'GROK_TASK'
...
```bash
node "${GROK_PLUGIN_ROOT}/scripts/grok-companion.mjs" review --target '<target from $ARGUMENTS>' --task-file '<path from $ARGUMENTS>' [other non-task flags from $ARGUMENTS, each substituted value single-quoted]
```
