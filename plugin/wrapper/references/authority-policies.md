...
| `consent-required` | `--integration direct` without per-repo setup consent (companion gate; JSON envelope on stdout). | Run `/grok:setup --integration direct` (optionally `--target <repo>`) once, then retry. |
