...
| `status` | N/A (pure local read of stored run state; never invokes Grok) | N/A | N/A | N/A | N/A | N/A | N/A |
...
| `turn-exhaustion` | Grok hit `--max-turns` without concluding. | Raise `--max-turns` or narrow the task so it needs fewer tool calls. |
| `cancelled` | Grok's own `stopReason` was `Cancelled` - typically a disallowed tool call was hard-denied under `permission-mode auto`. | Check whether the task needs a tool outside the mode's fixed allowlist; do not try to widen the allowlist yourself, that is a design decision. |
...
| `unexpected-edits` | A `review`/`reason` run reported a file change (read-only violation), or a `code` run touched files outside the verified worktree or inside the original checkout. | Treat as a safety violation, not a retry candidate; investigate before running again. |
