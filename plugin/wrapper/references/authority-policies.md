...
| `code` | **Mode-aware.** Bare wrapper / `--integration worktree` (and product `auto`/`review`): external git worktree the wrapper creates and verifies (never the current checkout); sandbox write root = worktree + private tmp. `--integration direct` (product default; no consent gate): operator **live checkout** (repo root); sandbox write root = repo root + private tmp (hardened-direct); no external worktree. ACP peer is a separate channel and is always external worktree at session time. | Editing and terminal tools (`write`, `search_replace`, `run_terminal_command`, `get_command_or_subagent_output`) plus read tools (`read_file`, `list_dir`, `grep`); confined by the sandbox to the mode's write root (worktree **or** repo root) | `workspace` (built-in; confines writes to cwd plus private temp; cwd is worktree or repo root per integration) | Permitted (D-NET) | Disabled | Opt-in per run (same as `review`) | `auto` |
...
| `handoff` | N/A (pure local read of code-run handoff artifacts by `--run-id`; never invokes Grok; never writes; never applies). Dual-condition ready: ready manifest + success envelope + patch rehash under run dir only. | N/A | N/A | N/A | N/A | N/A | N/A |
| `cleanup` | N/A (pure local read/remove of stored run state and, for code/verify, the worktree; never invokes Grok). Warns when removing integration-ready handoff artifacts. | N/A | N/A | N/A | N/A | N/A | N/A |
...
| `model-unavailable` | The requested model (default `grok-4.6`) is not in the selectable model list from `grok models`, or the run's effective model was not in the requested model family. Requesting `grok-4.6` never silently succeeds as `grok-4.5`. Preflight requires the product default to be selectable; it does not require `grok-4.5`. | Check account/model access; do not silently switch models. |
...
| `consent-required` | **Removed in 2.0.1** (historical class may still appear in old envelopes). Direct landing no longer gated. | N/A - omit; use isolation modes if you want a worktree. |
...
| `implementation-contract-invalid` | Bad/missing `--contract-file` (schema, symlink path, empty writeScopes, target mismatch) before Grok; also present-but-blank `--contract-file` / `--contract-file=` on code, direct, and peer-start (not silent no-contract). | Fix the contract JSON; pass a real path or omit the flag; reject symlink contract paths. |
| `write-scope-violation` | Post-Grok changed path (including rename `oldPath`) outside contract writeScopes. | Narrow the change or widen writeScopes; forensic handoff may still be written. |
| `unexpected-commit` | Worktree HEAD moved from `baseRevision` during the run. Worktree is not reset. | Inspect the worktree; do not auto-reset. |
| `artifact-generation-failure` | Patch/manifest capture failed (size, secret-shaped material, git error). | Check handoff blockers; re-run after fixing capture cause. |
| `artifact-integrity-failure` | Handoff rehash/load failed or patch path escaped the run dir. | Do not integrate; treat artifacts as untrusted. |
| `handoff-unavailable` | No code-run handoff artifacts for the requested runId (or wrong mode). | Ensure a hardened `code` run completed far enough to write artifacts. |
| `terminal-envelope-incomplete` | Manifest claimed ready but no success terminal envelope for the same runId. | Wait for terminal envelope / status recovery; dual-condition ready stays false. |
