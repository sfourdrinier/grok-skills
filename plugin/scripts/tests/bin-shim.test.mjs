...
import { makeFakeWrapper } from "./helpers/fake-wrapper.mjs";
...
function tempCwd() {
  return fs.mkdtempSync(path.join(os.tmpdir(), "grok-bin-shim-"));
}

function runShim(argv, { env = {}, cwd = process.cwd() } = {}) {
...
test("bin/grok-skills jobs exits 0 and prints jobs table header", () => {
  ...
  try {
    ...
    createJob(cwd, { kind: "review", mode: "review", runMode: "hardened" });
    const res = runShim(["jobs"], { cwd });
...
test("bin/grok-skills propagates nonzero exit for unknown wrapper mode", () => {
  ...
  try {
    ...
    const res = runShim(["status", "--run-id", RID], { env, cwd });
    assert.equal(res.error, undefined, `spawn failed: ${res.error}`);
