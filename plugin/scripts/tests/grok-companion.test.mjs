...
test("candidateWrapperPaths orders override, CLAUDE_PLUGIN_ROOT, PLUGIN_ROOT, then derived", () => {
...
test("resolveWrapperPath honors an explicit GROK_AGENT_WRAPPER override", () => {
  const real = resolveWrapperPath({});
  const resolved = resolveWrapperPath({ GROK_AGENT_WRAPPER: real });
  assert.equal(resolved, path.resolve(real));
});

test("wrapperNotFoundMessage is actionable and points at /grok:setup", () => {
  const message = wrapperNotFoundMessage({ GROK_AGENT_WRAPPER: "/nope/grok_agent.py" });
