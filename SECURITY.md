...
- **ACP peer channel (default; opt out with `GROK_DISABLE_ACP=1`):** the default
  ...
  **real commands** and sets `integration.ready` only from authoritative,
  ...
  integration mode via the **shared auto/peer apply spine** (best-effort patch
  integrity recheck under trusted local state, NUL-safe dirty status, numstat,
  dirty-overlap, `git apply --check`, apply, reverse-on-failure - see
  ...
  Peer-stop final envelope rewrite + job finalize happen before first stdout
  write; peer-stop is **not** completion-notification eligible. Progress-chunk
