...
4. **Wrapper owns safety.** All sandbox, auth-home, secret redaction, version pin, worktree isolation live in `plugin/wrapper/`. Plugin/companion is thin: resolve root, pass argv, relay **one** JSON envelope on stdout **verbatim**.
5. **Fail closed.** Unverified platform, version, sandbox, stream, or cache → classified failure, never silent success.
