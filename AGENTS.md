...
2. **Everything is DRY.** Never copy-paste logic, tables, patterns, prompts, or contracts. One source of truth; extract shared helpers; if it exists twice, delete one and call the other. Known single sources include: web defaults (`web_defaults.py`), citations parse, preflight cache, envelope field specs (`envelope.py`), secret patterns (`redaction.py`), run lifecycle CAS (`run_lifecycle.py`), tool allowlists (`modes/code._TOOLS` + `grokcli.effective_tools` / `WEB_TOOLS`), deny-write globs (`plugin/references/deny-write-globs.json` + Py/Node), git C-quoted paths (`plugin/references/git-c-quoted-path-vectors.json` + Py/Node), Grok CLI product defaults (`plugin/references/grok-cli-defaults.json` + `cli_defaults.py` / `cli-defaults.mjs`, including `is_same_model_family`), exclusive file lock (`filelock.py` used by `run.lock` and `worktree_lock.py`), companion flag values (`companion-args.mjs`), task staging (`task-file.mjs`), contract load (`load_optional_contract_arg`), apply integrity (`verifyPatchAgainstManifest` + `completeIntegrationApplyUnderLock`), manifests (`manifest.source.json` + `tools/gen-manifests.mjs`). **Known dual-source debt (do not grow it):** tracked in [issue #6](https://github.com/sfourdrinier/grok-skills/issues/6) (peer lease constant, `workspaceStateSegment`, flag-presence helpers, runner lifecycle orchestration, etc.). Next PR must be TDD and no-regression.
