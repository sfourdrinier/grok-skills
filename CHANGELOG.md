...
- **Patch integrity recheck before apply:** after apply-time handoff ready,
  auto re-verifies `implementation.patch` bytes/size/hash against the
  revalidated manifest (`verifyPatchAgainstManifest`, same SSOT as peer) and
  fails closed with `patch-integrity-failure` if substituted/corrupted.
- **Complete auto failure envelope:** when auto has no usable handoff envelope
  and the code leg produced missing/unparseable stdout, emit one full C4-shaped
  failure envelope (`schemaVersion`, `mode=code`, `status=failure`, stable
  `runId` or null, classified error, `response.integration.applied=false`)
  instead of a partial schemaVersion/mode/status-only object. Never invent
  success.
- **NUL-safe path inventory + quoted patch headers:** shared `path_inventory`
  ...
  `/dev/null`) without C-unquoting NUL-safe inventories.
